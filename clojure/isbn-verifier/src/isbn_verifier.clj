...
(defn- to-num-array [isbn]
  "convert to a number array; X will be converted to 10"
  (map #(if (= % \X) 10 (Integer/parseInt (str %))) isbn))
...
(defn isbn? [isbn]
  ...
  any dashes but don't allow anything else. Then pass the normalized isbn to to-num-array
